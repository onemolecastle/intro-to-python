...
for category in food_menu:
    most_expensive_item = max(category["items"], key=lambda item: item["price"])
...
vegetarian_items = [item["name"] for category in food_menu if category["category"] == "Main Course" for item in category["items"] if item["name"] == "Vegetable Stir-Fry"]
print("Vegetarian Items:", vegetarian_items)

# Exercise 6: Calculate the average price of items in the Desserts category
desserts_category = next(category for category in food_menu if category["category"] == "Desserts")
desserts_items_count = len(desserts_category["items"])
desserts_items_total_price = sum(item["price"] for item in desserts_category["items"])
...
expensive_items = [item["name"] for category in food_menu for item in category["items"] if item["price"] > 15]
...
total_items = sum(len(category["items"]) for category in food_menu)
