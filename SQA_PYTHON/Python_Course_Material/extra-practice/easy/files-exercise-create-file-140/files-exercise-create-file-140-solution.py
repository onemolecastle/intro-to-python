...
def create_file(filename):
    ...
    try:
        ...
        elif filename.endswith(".txt") == False:
            return "The filename must end with the extension '.txt'."
        elif re.search("[^a-zA-Z0-9_\.]", filename):
            return "Filename contains invalid characters."
        else:
            new_file = open(os.path.join(current_path, filename), "x")
            new_file.close()
            return "File created successfully."
    except FileExistsError:
        return "File already exists. File error."
